...
export class PropertyService {
	...
	public async getProperty(memberId: ObjectId, propertyId: ObjectId): Promise<Property> {
		...
		if (memberId) {
			...
			// // meLiked — ehtimol foydalanuvchining like qilganini tekshiradigan logika bu yerga yozilishi kerak
		}
	...
	public async propertyStatsEditor(input: StatisticModifier): Promise<Property> {
		...
		const updatedProperty = await this.propertyModel.findByIdAndUpdate(_id, { $inc: { [targetKey]: modifier } }, { new: true }).exec();
		if (!updatedProperty) {
